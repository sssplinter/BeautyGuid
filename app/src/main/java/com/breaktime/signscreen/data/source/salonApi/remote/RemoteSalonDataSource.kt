...
class RemoteSalonDataSource(private val salonService: SalonService) : SalonDataSource {
    override suspend fun getAllSalons(): List<SalonPreviewResponse> = withContext(Dispatchers.IO) {
    ...
    override suspend fun getSalonInfoById(salonId: Int): SalonInfo? = withContext(Dispatchers.IO) {
    ...
    override suspend fun getSalonNews(salonId: Int): List<SalonNews> =
        withContext(Dispatchers.IO) {
            return@withContext salonService.getSalonNews(salonId).body() ?: listOf()
        }
}